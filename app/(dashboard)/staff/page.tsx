...
const MemberCard = ({ member, onEdit, onDelete, onChangePassword, onStatusChange }: { member: any, onEdit: (m: any) => void, onDelete: (id: string) => void, onChangePassword: (m: any) => void, onStatusChange: (id: string, currentStatus: string) => void }) => (
  ...
  >
    <div className="flex items-start justify-between">
      <div className="flex items-center gap-3">
        <div className="relative">
          <div className="w-12 h-12 rounded-lg bg-slate-100 flex items-center justify-center text-slate-400 font-semibold text-lg border border-slate-200 group-hover:bg-indigo-50 group-hover:text-indigo-600 group-hover:border-indigo-100 transition-all duration-300">
        ...
        <div>
          <h3 className="text-sm font-semibold text-slate-900 leading-tight">{member.userId.fullName}</h3>
          <p className="text-[10px] font-medium text-indigo-600 mt-1 flex items-center gap-1.5">
            <Shield size={10} />
            {member.staffRole}
          </p>
        </div>
    ...
    <div className="space-y-2">
      <div className="flex items-center gap-2 text-[11px] text-slate-500 font-medium overflow-hidden">
      ...
      <div className="flex items-center gap-2 text-[11px] text-slate-500 font-medium">
        ...
        <div className="flex flex-wrap gap-1">
           <span className="px-1.5 py-0.5 bg-slate-50 border border-slate-100 rounded text-[9px] text-slate-600 font-medium">
              All Sites
            </span>
        </div>
      </div>
    ...
    <div className="pt-3 mt-auto border-t border-slate-50 flex items-center justify-between">
      <div>
        <p className="text-[9px] font-medium text-slate-400 mb-0.5">Account Status</p>
      ...
      >
        <div className={cn(
          ...
          member.userId.status === 'active' ? "translate-x-4" : "translate-x-0"
        )} />
      </button>
    </div>
  </motion.div>
        ...
        <div className="flex items-center gap-3">
          ...
          >
            <div className={cn(
              ...
              item.userId.status === 'active' ? "translate-x-3.5" : "translate-x-0"
            )} />
          </button>
        </div>
      )
