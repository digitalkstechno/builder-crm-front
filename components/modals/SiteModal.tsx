...
export default function SiteModal({
  ...
  const removeImage = (index: number) => {
    ...
    // Revoke URL to prevent memory leaks for new images
  ...
  return (
    <CommonDialog
      isOpen={isOpen}
      onClose={onClose}
      title={formData._id ? 'Refine Project' : 'Onboard Project'}
      maxWidth="max-w-2xl"
    >
            ...
            <div className="grid grid-cols-4 gap-3">
              <AnimatePresence>
                {formData.images?.map((img: any, idx: number) => (
                  ...
                  >
                    <img src={process.env.NEXT_PUBLIC_IMAGE_URL + img} alt="" className="w-full h-full object-cover" />
                    <button 
                      type="button"
            ...
            <div className="grid grid-cols-4 gap-3">
              <AnimatePresence>
                {formData.images?.map((img: any, idx: number) => (
                  ...
                  >
                    <img
                      src={typeof img === 'string' ? `${process.env.NEXT_PUBLIC_IMAGE_URL}${img}` : img.preview}
                      alt=""
